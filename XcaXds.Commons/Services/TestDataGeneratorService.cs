...
public static class TestDataGeneratorService
{
    public static List<RegistryObjectDto> GenerateRegistryObjectsFromTestData(Test_DocumentReference documentEntryValues, int amount)
    {
        ...
        while (maxCount < amount)
        {
            ...
            {
                Author = GetRandomAuthors(documentEntryValues.PossibleDocumentEntryValues.Authors, Random.Shared.Next(4))?.DistinctBy(aut => aut.Person?.Id).ToList(),
                ...
                ConfidentialityCode = PickRandomAmount(documentEntryValues.PossibleDocumentEntryValues.ConfidentialityCodes, Random.Shared.Next(4)).DistinctBy(confCode => confCode.Code).ToList(),
