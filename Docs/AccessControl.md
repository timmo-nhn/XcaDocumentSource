...
## XACML Request
The XACML-Request is generated by the **PEP** from fields in the **SAML-token** or **Json Web Token (JWT)**. Every eligible attribute is transformed from **HL7 XML Schema Instance (XSI)** snippet or similar codeable datatype, into a custom `CodedValue` type, which is then added as up to three attributes in the XACML-request, appending `:code`, `:codeSystem` or `:displayName` to the end of a give attribute.
...
Here, `code` and `codeSystem` are transformed by the **PEP** into their own separate attributes.
